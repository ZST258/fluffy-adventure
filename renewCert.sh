...
if [ "${domainRecord[${i}]}" == "@" ]; then
  ...
  bash /root/.acme.sh/acme.sh --install-cert -d "${topDomain}" --ecc --fullchain-file "${installPath}/fullchain.cer" --key-file "${installPath}/private.key"
  if [ ! -f "/root/.acme.sh/${topDomain}_ecc/fullchain.cer" ]; then
    echo -e "\033[0;31;1m ${topDomain}证书安装失败,请手动安装! \033[0m"
    rm -f "${installPath}/fullchain.cer"
    rm -f "${installPath}/private.key"
    fail=$[$fail+1]
  else
  ...
  bash /root/.acme.sh/acme.sh --install-cert -d "${domainRecord[${i}]}.${topDomain}" --ecc --fullchain-file "${installPath}/fullchain-${domainRecord[${i}]}.cer" --key-file "${installPath}/private-${domainRecord[${i}]}.key"
  if [ ! -f "/root/.acme.sh/${domainRecord[${i}]}.${topDomain}_ecc/fullchain.cer" ]; then
    ...
    rm -f "${installPath}/fullchain-${domainRecord[${i}]}.cer"
    rm -f "${installPath}/private-${domainRecord[${i}]}.key"
    fail=$[$fail+1]
  else
    successDomain[${index}]=${domainRecord[${i}]}
